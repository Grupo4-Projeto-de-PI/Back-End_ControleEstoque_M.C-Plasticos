...
class LogEntradaController(val repositorio: LogEntradaRepositorio) {
    ...
    @GetMapping
    fun listar(): List<LogEntrada> = repositorio.findAll()

    @GetMapping("/{id}")
    fun buscarPorId(@PathVariable id: Int): ResponseEntity<LogEntrada> {
    ...
    @PostMapping
    fun criar(@RequestBody logEntrada: LogEntrada): ResponseEntity<LogEntrada> {
        val salvo = repositorio.save(logEntrada)
        return ResponseEntity.ok(salvo)
    }

    @PutMapping("/{id}")
    fun atualizar(@PathVariable id: Int, @RequestBody logEntrada: LogEntrada): ResponseEntity<LogEntrada> {
